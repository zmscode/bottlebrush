...
- [x] **BigInt:** arithmetic (needs a bignum core — implement or vendor), `asIntN/asUintN`, `toString(radix)`, mixed-type TypeErrors. **DONE: backed by `std.math.big.int.Managed` (true arbitrary precision, not an i64 stub); `BigInt.asIntN`/`asUintN` (wrap mod 2^bits), `toString(radix 2..36)`, and mixed BigInt+Number → TypeError ("cannot mix BigInt and other types"). Remaining are edge cases (audit F6 ~23): `**` RangeError, wrap boundaries, `toPrimitive` hints.**
...
- [x] **Map / Set / WeakMap / WeakSet:** hash by SameValueZero, insertion-order iteration, `@@species`; Weak variants integrate with GC (do **not** keep keys alive — needs GC ephemeron support; coordinate with `gc/heap.zig`). **DONE: Map/Set (SameValueZero, insertion order) plus WeakMap/WeakSet/WeakRef — the collector runs an ephemeron pass (`gc.zig` Heap.collect) that does NOT keep weak keys/targets alive and clears dead entries (`weak_target` → `.hole`). Symbols may be weak keys. STILL OUTSTANDING: `@@species` on the Map/Set constructors is not wired; and `WeakMap.prototype.getOrInsert*` (a newer proposal in the vendored corpus) is absent (~4 fails).**
